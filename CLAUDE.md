...
user-facing**: `README.md`, `SKILL.md` / any skill, public docs, and
code/header comments. Describe the behaviour or the code directly instead
(e.g. "the dual-core split in `fb_core1_dispatch`", not "the dual-core
split from Story N.M"). When you touch a comment that cites an epic/story,
rephrase it. Cross-references *between* the epic docs themselves are fine
— they're not committed. This is a hard rule.
...
2. Builds the Atari ST target (`target/atarist/build.sh`) via `stcmd make`. The MOVEM-loop cart→ST screen copy is no longer a separate file: it's emitted inline in `target/atarist/src/userfw.s` via the `FBDRV_INLINE` macro (the old hand-written `src/fbdrv.s`, and the `gen_fbdrv.py` generator before it, were both dropped). Enforces a **16 KB hard limit** on `BOOT.BIN` (the cartridge code budget — `CART_CARTRIDGE_CODE_SIZE` in `rp/src/include/cart_shared.h`, mirrored as `CARTRIDGE_CODE_SIZE` in `target/atarist/src/main.s`); a build that exceeds it aborts with `ERROR: cartridge code is N bytes; limit is 16384`. A separate copy (`FIRMWARE.IMG`) is then padded to 64 KB to fill the entire shared region, and `firmware.py` converts it into `rp/src/include/target_firmware.h` (a C byte array embedded in the RP firmware).
...
| --- | --- | --- | --- |
| `$FA0000` | cartridge image | 16 KB | m68k header + main.s (≤ 2 KB) + userfw.s (~6 KB code + the inline `FBDRV_INLINE` MOVEM blit; ~8.3 KB total). Read-only from m68k. |
